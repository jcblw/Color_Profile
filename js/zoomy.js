...
// global zoomys state, Indexed, 0 = no zoom, 1 = zoom;
	...
	//defaults && option list
	    var defaults = {
		    ...
		    change = {
				//Needs to be revised to where the offset is determined better e.offsetX is giving me bogus numbers 
				borderColor : function(x){				    
				    ...
				    for(var i = 0; found; i += 1){
					if(x >= color_form.pos[i] ){
					    ZoomyS.currentColor = i;
					}else{
					    found = false;
					    $('.zoomy').css('border-color', '#' + color_form.colors[ZoomyS.currentColor]);
					}
				    }
				    //console.log(ZoomyS.currentColor);
				},
